...
class Gradient(Mesh):
    ...
    def _find_neighbor(self):
        ...
        for node, df in self._obj.groupby("node_id"):
            ...
            nodes = (
                self._obj.loc[self._obj.index.isin(elidx, level="element_id")]
                .index.get_level_values("node_id")
                .to_numpy()
            )
    ...
    def _calc_lst_sqr(self):
        ...
        for node, row in zip(
            groups.first().index,
            np.nditer(self._node_data.T, flags=["external_loop"], order="F"),
        ):
...
class Gradient3D(Mesh):
    ...
    def _compute_gradient_hexahedral(self, df):
        ...
        for node_index, xi in enumerate(
            [
                (0, 0, 0),
                (1, 0, 0),
                (1, 1, 0),
            ...
            J11 = xi3 * (-x51 * (1 - xi2) + x61 * (1 - xi2) + x71 * xi2 - x81 * xi2) + (
                1 - xi3
            ) * (-x11 * (1 - xi2) + x21 * (1 - xi2) + x31 * xi2 - x41 * xi2)
            J21 = xi3 * (-x52 * (1 - xi2) + x62 * (1 - xi2) + x72 * xi2 - x82 * xi2) + (
                1 - xi3
            ) * (-x12 * (1 - xi2) + x22 * (1 - xi2) + x32 * xi2 - x42 * xi2)
            J31 = xi3 * (-x53 * (1 - xi2) + x63 * (1 - xi2) + x73 * xi2 - x83 * xi2) + (
                1 - xi3
            ) * (-x13 * (1 - xi2) + x23 * (1 - xi2) + x33 * xi2 - x43 * xi2)
            J12 = xi3 * (-x51 * (1 - xi1) - x61 * xi1 + x71 * xi1 + x81 * (1 - xi1)) + (
                1 - xi3
            ) * (-x11 * (1 - xi1) - x21 * xi1 + x31 * xi1 + x41 * (1 - xi1))
            J22 = xi3 * (-x52 * (1 - xi1) - x62 * xi1 + x72 * xi1 + x82 * (1 - xi1)) + (
                1 - xi3
            ) * (-x12 * (1 - xi1) - x22 * xi1 + x32 * xi1 + x42 * (1 - xi1))
            J32 = xi3 * (-x53 * (1 - xi1) - x63 * xi1 + x73 * xi1 + x83 * (1 - xi1)) + (
                1 - xi3
            ) * (-x13 * (1 - xi1) - x23 * xi1 + x33 * xi1 + x43 * (1 - xi1))
            J13 = (
                -x11 * (1 - xi1) * (1 - xi2)
                - x21 * xi1 * (1 - xi2)
                - x31 * xi1 * xi2
            ...
            df.iloc[node_index, 4:7] = self._compute_gradient_hexahedral_single_node(
                xi, nodal_values, Jinv
            )
    ...
    def _compute_gradient_simplex(self, df):
        ...
        for node_index in range(4):
            # loop over derivative index d/dx_k
            df.iloc[node_index, 4:7] = self._compute_gradient_simplex_single_node(
                node_index, nodal_values, Jinv
            )
    ...
    def _compute_gradient(self, df):
        ...
        # if the element is a 4-node simplex/tetrahedral element
            ...
            warnings.warn(
                f"Element has {len(df)} nodes, which is not a valid 3D element. "
    ...
    def gradient_of(self, value_key):
        ...
        df_grad = df.groupby("element_id", group_keys=False).apply(
