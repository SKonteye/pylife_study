...
class P_RAM:
    ...
    def _compute_values(self):
        ...
        self._collective.loc[self._collective["S_m"] >= 0, "k"] = self._M_sigma * (
            self._M_sigma + 2
        )
        self._collective.loc[self._collective["S_m"] < 0, "k"] = (
            self._M_sigma / 3 * (self._M_sigma / 3 + 2)
        )

        # compute (σ_a + k*σ_m)
        self._collective["discriminant"] = (
            self._collective["S_a"] + self._collective["k"] * self._collective["S_m"]
        )

        # compute P_RAM according to eq. (2.6-82)
        self._collective["P_RAM"] = np.where(
            self._collective["discriminant"] >= 0,
            np.sqrt(
                self._collective["discriminant"]
                * self._collective["epsilon_a"]
                * self._assessment_parameters.E
            ),
            0.0,
        )
...
class P_RAJ:
    ...
    def __init__(
        ...
        if not (
            (self._collective.run_index.unique() == np.array([1, 2])).all()
            or (self._collective.run_index.unique() == np.array([2])).all()
        ):
            raise AssertionError
    ...
    def _compute_values(self):
        ...
        self._ramberg_osgood_relation = pylife.materiallaws.rambgood.RambergOsgood(
            E=self._assessment_parameters.E,
            K=self._assessment_parameters.K_prime,
            n=self._assessment_parameters.n_prime,
        )
        self._collective["epsilon_open_ein"] = (
            self._collective.epsilon_min
            + self._ramberg_osgood_relation.delta_strain(
                self._collective.S_open - self._collective.S_min
            )
        )
    ...
    def _compute_S_open(self):
        ...
        self._assessment_parameters.R_p02 = (
            0.002**self._assessment_parameters.n_prime
            * self._assessment_parameters.K_prime
        )
        self._assessment_parameters.S_F = 0.5 * (
            self._assessment_parameters.R_p02 + R_m
        )
        ...
        self._collective["A_m"] = np.where(
            self._collective.S_m < 0,
            0.4 - self._M_sigma / 4,
            0.47
            * (1 - 1.5 * self._M_sigma)
            * (1 + self._collective.R) ** (1 + self._collective.R + self._M_sigma),
        )

        # compute coefficients according to eq. (2.8-63)
        self._collective["A_0"] = (
            0.535
            * np.cos(
                np.pi / 2 * self._collective.S_max / self._assessment_parameters.S_F
            )
            + self._collective.A_m
        )
        self._collective["A_1"] = (
            0.344 * self._collective.S_max / self._assessment_parameters.S_F
            + self._collective.A_m
        )
        self._collective["A_3"] = 2 * self._collective.A_0 + self._collective.A_1 - 1
        self._collective["A_2"] = (
            1 - self._collective.A_0 - self._collective.A_1 - self._collective.A_3
        )

        # compute crack opening stress
        self._collective["S_open_factor"] = np.where(
            (0 <= self._collective.R) & (self._collective.R < 1),
            self._collective.A_0
            + self._collective.A_1 * self._collective.R
            + self._collective.A_2 * self._collective.R**2
            + self._collective.A_3 * self._collective.R**3,
            np.where(
                self._collective.R < 0,
                self._collective.A_0 + self._collective.A_1 * self._collective.R,
                1,
            ),
        )

        self._collective["S_open"] = (
            self._collective.S_max * self._collective.S_open_factor
        )

        # remove temporary columns
        self._collective.drop(
            columns=["A_0", "A_1", "A_2", "A_3", "S_open_factor"], inplace=True
        )
    ...
    def _calculate_P_RAJ(self, delta_S_eff, delta_epsilon_eff):
        ...
        P_RAJ = 1.24 * np.power(
            delta_S_eff, 2
        ) / self._assessment_parameters.E + 1.02 / np.sqrt(
            self._assessment_parameters.n_prime
        ) * delta_S_eff * (
    ...
    def _calculate_fatigue_limit_variables(self, D_akt):
        ...
        a_0 = (
            a_end ** (1 - m)
            - (1 - m) * C * self._component_woehler_curve_P_RAJ.P_RAJ_Z**m
        ) ** (1 / (1 - m))
        ...
        # eq. (2.9-116)
        denominator = (
            ((a_end ** (1 - m) - a_0 ** (1 - m)) * D_akt + a_0 ** (1 - m))
            ** (1 / (1 - m))
            + delta_J_eff_th
            / self._component_woehler_curve_P_RAJ.fatigue_strength_limit
            - a_0
        )
    ...
    def _compute_crack_opening_loop(self):
        ...
        assessment_point_index = self._collective[
        ...
        last_index_of_first_run = self._collective[
            (self._collective["run_index"] == 1)
            & (self._collective["run_index"].shift(-1) == 2)
        ].index
        ...
        if len(last_index_of_first_run) == 0:
            ...
            # eq. (2.9-123)
            self._assessment_parameters.P_RAJ_D_e = (
                self._P_RAJ_D_0
                * (a_0 + l_star)
                / (self._assessment_parameters.a_end + l_star)
            )
            ...
            # eq. (2.9-125)
            self._assessment_parameters.P_RAJ_klass_max = self._calculate_P_RAJ(
        ...
        for index, group in self._collective.groupby("hysteresis_index"):
            ...
            is_damage_in_current_hysteresis = pd.Series(
                True, index=assessment_point_index
            )
            ...
            condition_2 = (~condition_1) & (
                (epsilon_max_alt_SP < group.epsilon_max_LF)
            ...
            epsilon_min_alt_SP.mask(
                condition_2, other=group.epsilon_min_LF, inplace=True
            )
            epsilon_max_alt_SP.mask(
                condition_2, other=group.epsilon_max_LF, inplace=True
            )
            ...
            condition_4a = condition_4 & (
                group.S_a >= 0.4 * self._assessment_parameters.S_F
            )
            ...
            epsilon_min_alt_SP.mask(
                condition,
                other=np.minimum(epsilon_min_alt_SP, group.epsilon_min),
                inplace=True,
            )
            epsilon_max_alt_SP.mask(
                condition,
                other=np.maximum(epsilon_max_alt_SP, group.epsilon_max),
                inplace=True,
            )
            ...
            self._collective.loc[
                self._collective.index.get_level_values("hysteresis_index") == index,
                "epsilon_open",
            ] = epsilon_open.to_numpy()

            # store the used values of epsilon_min_alt
            self._collective.loc[
                self._collective.index.get_level_values("hysteresis_index") == index,
                "epsilon_min_alt_SP",
            ] = epsilon_min_alt_SP.to_numpy()
            self._collective.loc[
                self._collective.index.get_level_values("hysteresis_index") == index,
                "epsilon_max_alt_SP",
            ] = epsilon_max_alt_SP.to_numpy()
            ...
            def f(S_close, S_max, epsilon_max, epsilon_open):
                return self._ramberg_osgood_relation.delta_strain(S_max - S_close) - (
                    epsilon_max - epsilon_open
                )
            ...
            def fprime(S_close, S_max, epsilon_max, epsilon_open):
                ...
                return -self._ramberg_osgood_relation.tangential_compliance(
                    delta_stress / 2
                )
            ...
            S_close.mask(
                epsilon_open < group.epsilon_min, other=group.S_min, inplace=True
            )
            ...
            # eq. (2.9-110)
            delta_epsilon_eff = np.where(
                epsilon_open < group.epsilon_min,
            ...
            ] = is_damage_in_current_hysteresis.to_numpy()
            ...
            D = np.where(group.is_closed_hysteresis, 1.0 / N, 0.5 / N)
            ...
            epsilon_open_alt.mask(
                condition,
                other=group.epsilon_open_ein
                - (group.epsilon_open_ein - epsilon_open_alt) * np.exp(-15 / N),
                inplace=True,
            )

            # for the other cases 1,2, and 4:
            epsilon_open_alt.mask(
                case_name.isin(["1", "2", "4"]), other=epsilon_open, inplace=True
            )
            ...
            self._collective.loc[
                self._collective.index.get_level_values("hysteresis_index") == index,
                "epsilon_open_alt",
            ] = epsilon_open_alt.to_numpy()

            # compute helper variables for fatigue limit
            a_0, delta_J_eff_th, P_RAJ_D = self._calculate_fatigue_limit_variables(
                D_akt
            )
